...
<form action="{{url('/updateproduct',$data->id)}}" method="post" enctype="multipart/form-data">
    ...
    <div style="padding: 15px;">
        ...
        <select  style="color:black; height:35px; width: 215px;" class="" name="category">
            <option >{{$data->category =='0'? 'selected':'men'}}</option>
            <option >{{$data->category =='1'? 'selected':'women'}}</option>
            <option >{{$data->category =='2'? 'selected':'kids'}}</option>
        </select>
    </div>
